...
class parse_input:
    ...
    def parseMove(self, input):
        if re.match("^[RDTAC]?[a-h][1-8]$|^[DTAC][a-h][a-h][1-8]$|\
            ^[DTAC][1-8][a-h][1-8]$", input) or input in {"0-0", "0-0-0"} or \
            re.match("^[a-h][18][DTAC]$ |^[a-h][x][a-h][1-8][DTAC]", input) or \
            re.match("^[RDTAC][x][a-h][1-8]$|^[a-h][x][a-h][1-8]$| \
            ^[DTAC][a-h][x][a-h][1-8]$|^[DTAC][1-8][x][a-h][1-8]$", input):
            return input
        else:
            return -1
